...
struct win32_sound_output
{
  ...
  int BytesPerSample;
  int BufferSize;
  uint32_t SampleIndex; /* goes up for ever... */
  bool SoundPlaying;
};
...
Win32FillSoundBuffer (win32_sound_output *SoundOutput, DWORD ByteToLock, DWORD BytesToWrite)
{
  ...
  {
    float t = (float) SoundOutput->SampleIndex / (float) SoundOutput->WavePeriod * 2.0f * pi;
         ...
         LPSTR CommandLine, int ShowCode)
{
  ...
  if (RegisterClass (&WindowClass))
  {
    ...
    if (Window)
    {
      ...
      SoundOutput.BufferSize = SoundOutput.SamplesPerSecond * sizeof(int16_t) * 2;
      SoundOutput.SampleIndex = 0; /* goes up for ever... */
      SoundOutput.SoundPlaying = false;

      Win32LoadDirectSound (Window, SoundOutput.SamplesPerSecond,
      ...
      while (Running)
      {
        ...
        DWORD PlayCursor; /* estimate of where the sound card is playing the buffer */
        DWORD WriteCursor;

        if (!SoundOutput.SoundPlaying && /* we only write the buffer, it's the same tone for now */
        ...
        if (!SoundOutput.SoundPlaying)
        {
          SecondaryBuffer->Play(0, 0, DSBPLAY_LOOPING); /* circular buffer */
          SoundOutput.SoundPlaying = true;
        }
